...
const kdutifyMatcher = require('./matcher/tag')
const kdutifyAttrsMatcher = require('./matcher/attr')
...
function injectStylesSSR (imports) {
  ...
  if (styles.size) {
    return `
render._kdutifyStyles = function (component) {
  if (process.env.KDU_ENV === 'server') {
    const options = typeof component.exports === 'function'
      ? component.exports.extendOptions
      : component.options
    const existing = options.beforeCreate
    const hook = function () {
${[...styles].map((style) => `      require('kdutify/${style}').__inject__(this.$ssrContext)`).join('\n')}
    }
...
module.exports = async function (content, sourceMap) {
  ...
  options.match.push(kdutifyMatcher)
  options.attrsMatch.push(kdutifyAttrsMatcher)


  const qs = new URLSearchParams(this.resourceQuery)
  if (qs.has('kdu') && qs.get('type') === 'template') {
    ...
    this.addDependency(this.resourcePath)
    const newContent = 'if (render._kdutifyStyles) { render._kdutifyStyles(component) }'
